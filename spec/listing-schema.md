...
"media": {
  ...
  "gallery":   [ { "url": "…", "sha256": "…", "category": "exterior", "width": …, "height": …, "caption": "…", "sort": 1, "thumbnail_url": "…" } ],
  "layouts":   [ { "url": "…", "sha256": "…", "width": …, "height": …, "caption": "GA", "sort": 1, "thumbnail_url": "…" } ],
...
- The exception to the single-URL rule is **thumbnails** — small, authority-served renditions for contexts where a consumer reasonably wants a preview without running an image pipeline (share cards, listing pickers, pre-import/curation screens, browser-side renderers, quick integrations). `profile.thumbnail_url` is **required (non-null) whenever `profile` is present**. `gallery[].thumbnail_url` and `layouts[].thumbnail_url` are **nullable**: one authority-served small rendition per image, or `null` meaning no small rendition is served and consumers fall back to deriving from `url`. There is no size negotiation; no dimensions are mandated, ~400–640 px on the long edge is the sensible range. Thumbnails carry no checksum — they are convenience renditions, not archival media; cache-and-verify semantics attach to `url`. They inherit the listing's `usage` terms like any cached media. When non-null, a `thumbnail_url` MUST be a rendition of the same image as its `url` (LS-16), and consumers SHOULD prefer it for grid and preview contexts. These thumbnails are the complete set of authority-served renditions — all other sizes derive from `url`. *(Gallery/layout thumbnails added from implementer feedback, 2026-08: both existing implementations' pre-import curation screens needed gallery previews before any media caching is possible — see the resolved-questions record below.)*
...
```json
{
  ...
  "media": {
    ...
    "gallery": [
      { "url": "https://authority.example/media/listings/018f6d2e-9f0a-7cc3-a1b2-3c4d5e6f7a8b/02.jpg", "sha256": "78a5b7b942ebaa3ffd8f54b695bda9619787b0ebd7ac7fffc29642e5d62a9695", "category": "exterior", "width": 4000, "height": 2667, "caption": "Aft deck", "sort": 1, "thumbnail_url": "https://authority.example/media/listings/018f6d2e-9f0a-7cc3-a1b2-3c4d5e6f7a8b/02-thumb.jpg" }
    ],
    "layouts": [],
    "videos": [
...
2. **Media single-URL rule** — **resolved** (maintainer ruling from direct feed-integration experience, plus one confirming response): single large image per item, receiver resizes to its own needs; providers' pre-generated size sets never standardise across sources. One exception applied: `profile.thumbnail_url`, mandatory whenever imagery exists, for share/preview contexts; generated placeholder images are prohibited (`profile: null` is the only correct "no image"). *Amended 2026-08 on implementer feedback — the revisit clause this resolution carried has fired*: the single-URL resolution stands (negotiated/standardised size sets stay rejected), but the thumbnail exception is widened from profile-only to nullable `thumbnail_url` on gallery and layout items. Both existing implementations' pre-import curation screens needed gallery previews before media caching is possible — the consumer-side case the original ruling had no evidence from.
