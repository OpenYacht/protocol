...
- **Acceptance policy is local, and belongs on the partner rather than the listing.** Synchronising a listing is not publishing it: `updated_since` delivers everything the authority shares with you, and holding a provenance-tracked copy carries no obligation to display it. Consumers that want a curation step are better served expressing it as a per-partner policy — accept everything, accept what matches the operator's own criteria, or hold for review — than as a per-listing queue, which does not survive a partner with real inventory turnover and collides with the 24-hour update obligation in `yacht-identity.md`. Field-group gating (`LS-14`) means a legitimately shared listing may arrive with pricing or exact location withheld, so a completeness check is usually the useful criterion. Four things stay outside any such policy: a listing flagged in an unresolved vessel-identity conflict, a listing whose `usage.display` is false, the staleness rules for an unreachable partner, and sanitisation of rich text — required in every case, and where no person reviews a listing before it appears, the only thing standing between a partner's description and your rendered page. On the first of those: the conformance obligation attaches to hard-identifier collisions, but a policy that consults that flag alone will publish a duplicate of a vessel it already holds whenever no HIN or IMO is present. Holding an unconfirmed `builder + model + year_built + loa_m` candidate for the same review is operator prudence rather than a requirement of this specification — and an auto-publish path is the cheapest place to apply it. *(Added from implementer feedback, 2026-08: existing implementations had uniformly built a per-listing import queue — conformant, but not a shape that survives a partner with real inventory turnover, and the one integrators would copy.)*
